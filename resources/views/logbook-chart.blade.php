<div class="d-flex flex-wrap flex-stack p-5">
		...
		<div class="d-flex flex-wrap my-1">
			<!-- <a href="#" class="btn btn-sm @if($talent->superlog1) btn-secondary @endif btn-primary me-2" data-bs-toggle="modal" data-bs-target="#kt_modal_assessment" data-talent="{{$talent->id}}" data-series="1">Baseline Assessment</a> -->
			<a href="#" class="btn btn-sm @if($talent->superlog2) btn-secondary @endif btn-primary me-2" data-bs-toggle="modal" data-bs-target="#kt_modal_assessment" data-talent="{{$talent->id}}" data-series="2">1st Year Assessment</a>
			<a href="#" class="btn btn-sm @if($talent->superlog3 || !$talent->superlog2) btn-secondary @endif btn-primary me-2" data-bs-toggle="modal" data-bs-target="#kt_modal_assessment" data-talent="{{$talent->id}}" data-series="3">2nd Year Assessment</a>
	...
	<div class="col-lg-6">
		...
		<div class="card card-flush h-lg-100 mb-5">
			...
			<div class="card-header">
				...
				<div class="card-toolbar">
					...
					<div class="menu menu-sub menu-sub-dropdown w-250px w-md-300px" data-kt-menu="true" id="kt_menu_61de0f5f83814">
						...
						<!--end::Menu separator-->
						{!! Form::open(array('route' => ['delete.talentlogbook'], 'method'=>'POST', 'id' => 'kt_add_mark_form')) !!}
			...
			<div class="card-body p-5 ps-1">
				@php $x=0; @endphp
				@php $cats = array();$catm1 = array();$catm2 = array(); @endphp
				@foreach($logcategory as $cat)
				@php $x++;@endphp
				<!--begin::Notice-->
				<div class="fs-5 text-primary fw-bolder mb-7 ps-7">{{$cat->id}}. {{$cat->name}}</div>
				<!--end::Notice-->
				<!--begin::Wrapper-->
				<div class="d-flex flex-row">
					<!--begin::Chart-->
					<canvas id="kt_chartjs_{{$cat->id}}" class="w-50 h-100 me-5"></canvas>
					<!--end::Chart-->
					<!--begin::Labels-->
					<div class="d-flex flex-column justify-content-center flex-row-fluid">
						<table id="kt_table_users" class="table border table-rounded g-3 table-row-bordered table-row-gray-300 fs-7">
						    ...
						    <tbody class="text-gray-800">
						    	@php $i=0; $subs = array();$subm1 = array();$subm2 = array();  @endphp
						    	...
						    	$subs[] = $cat->code.$i;
						    	if($talent->selflog3){
							    	$subm1[] = ($talent->applicant->selflog)?$talent->applicant->selflog->$q??'0':$talent->selflog2->$q??'0';
						    	...
						    	@endphp
						    	<tr>
						    		<td>{{$cat->code}}{{$i}} {{$log->item}}</td>
						    		<td class="text-center text-danger">{{$talent->applicant->selflog->$q??''}}</td>
						    		<td class="text-center {{($talent->selflog3)?'text-gray-800':'text-primary'}}">{{$talent->selflog2->$q??''}}</td>
						    		<td class="text-center {{($talent->selflog3)?'text-primary':'text-gray-800'}}">{{$talent->selflog3->$q??''}}</td>
						    	</tr>
						    	@endforeach
						    	@php  $cats[] = $subs;$catm1[] = $subm1;$catm2[] = $subm2;@endphp
						    </tbody>
						</table>
					</div>
					<!--end::Labels-->
	...
	<div class="col-lg-6">
		...
		<div class="card card-flush h-lg-100 mb-5">
			...
			<div class="card-header">
				...
				<div class="card-toolbar">
					...
					<div class="menu menu-sub menu-sub-dropdown w-250px w-md-300px" data-kt-menu="true" id="kt_menu_61de0f5f83814">
						...
						<div class="separator border-gray-200"></div>
						<!--end::Menu separator-->
						{!! Form::open(array('route' => ['delete.supervisorlogbook'], 'method'=>'POST', 'id' => 'kt_add_mark_form')) !!}
    					<input type="hidden" name="talent_id" value="{{$talent->id}}">
    					<input type="hidden" name="supervisor_id" value="{{$talent->superlog1->supervisor_id??''}}">
						<!--begin::Form-->
						<div class="px-7 py-5">
							<!--begin::Input group-->
							<div class="mb-10">
			...
			<div class="card-body p-5 ps-1">
				@php $x=0; @endphp
				@php $cats1 = array();$cats2 = array(); @endphp
				@foreach($logcategory as $cat)
				@php $x++;@endphp
				<!--begin::Notice-->
				<div class="fs-5 text-primary fw-bolder mb-7 ps-7">{{$cat->id}}. {{$cat->name}}</div>
				...
				<div class="d-flex flex-row">
					...
					<!-- <div class="position-relative d-flex flex-center h-175px w-175px me-15 mb-7">
						...
						<canvas id="kt_chartjs_{{$cat->id}}" class="w-50 h-300px"></canvas>
					
					</div> -->
					<canvas id="kt_chartjs_super_{{$cat->id}}" class="w-50 h-100 me-5"></canvas>
					<!--end::Chart-->
					<!--begin::Labels-->
					<div class="d-flex flex-column justify-content-center flex-row-fluid">
						<table id="kt_table_users" class="table border table-rounded g-3 table-row-bordered table-row-gray-300 fs-7">
						    ...
						    <tbody class="text-gray-800">
						    	@php $i=0; $subm1 = array();$subm2 = array();  @endphp
								...
								if($talent->superlog3){
						    	    $subm1[] = isset($ref[0]) && isset($ref[1])?$subref:$talent->superlog2->$q??'0';
						    	...
						    	<tr>
						    		<td>{{$cat->code}}{{$i}} {{$log->item}}</td>
						    		<td class="text-center text-danger">{{$subref}}</td>
						    		<td class="text-center {{($talent->superlog3)?'text-gray-800':'text-primary'}}">{{$talent->superlog2->$q??''}}</td>
						    		<td class="text-center {{($talent->superlog3)?'text-primary':'text-gray-800'}}">{{$talent->superlog3->$q??''}}</td>
						    	</tr>
						    	@endforeach
						    	@php  $cats1[] = $subm1;$cats2[] = $subm2;@endphp
						    </tbody>
						</table>
					</div>
					<!--end::Labels-->
...
var KTAppEcommerceSavelogbook = function() {
	...
	return {
        init: function() {
                ...
                e = document.getElementById("kt_chartjs_{{$cat->id}}");
                e && new Chart(e, {
                    type: 'radar',
		    data: {
			    ...
			    label: ("{{$talent->selflog3}}"==!1)?'CM-1':'CM-2',
		    ...
		    defaults:{
                ...
                e = document.getElementById("kt_chartjs_super_{{$cat->id}}");
		    ...
		    data: {
			    ...
			    label: ("{{$talent->superlog3}}"==!1)?'CM-1':'CM-2',
...
KTUtil.onDOMContentLoaded((function() {
    ...
    $("#kt_modal_assessment").on('show.bs.modal', function (e) {
        ...
        $.get( "{{url('/supervisor/logbook')}}/"+series+'/'+id+'/modal', function( data ){
