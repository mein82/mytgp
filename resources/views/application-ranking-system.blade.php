<x-app-layout activemenu="sec-ars">
<x-slot name="header">
	Applicant
</x-slot>
...
<div class="card-body py-10">
	<div class="d-flex flex-stack flex-wrap mb-5">
		...
		<div class="d-flex justify-content-end" data-kt-docs-table-toolbar="base">
			@if(Auth::user()->group_id<=2 || (Auth::user()->group_id==5 && is_null(Auth::user()->programme_id)))
			<!--begin::Filter-->
        ...
        @php $count = 0; @endphp
            <tr>
                <td>{{$item->name}}<br/>
                    ...
                    <span class="text-gray-500 fs-8">{{$item->organization}}</span><br/>
					@if(Auth::user()->group_id<=2 || (Auth::user()->group_id==5 && is_null(Auth::user()->programme_id)))
					<span class="text-gray-500 fs-8">({{$item->applicant->programme->name??''}})</span>
					@endif
                </td>
                <td>@if(checkComplete($item->applicant)>=8)
                ...
                <td class="text-center">{{$item->applicant->ars}}</td>
                <td class="text-center">{{$item->applicant->yearofservice}}</td>
                </tr>
        @endforeach
    </tbody>
</table>
