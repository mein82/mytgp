...
<div class="card mb-7">
	...
	<div id="kt_modal_users_search_handler" data-kt-search-keypress="true" data-kt-search-min-length="3" data-kt-search-enter="false" data-kt-search-layout="inline">
		...
		<div class="py-5">
			...
			<form action="{{route('add.referees')}}" method="POST" id="kt_modal_users_search_form">
			@csrf
				<!--begin::Users-->
				...
				<table id="kt_project_users_table" class="table table-row-bordered table-row-dashed gy-4 align-middle fw-bolder">
					...
					<tbody class="fs-6">
						...
						<tr>
								...
								<div class="d-flex align-items-center">
									...
									<div class="me-5 position-relative">
										...
										<div class="symbol symbol-35px symbol-circle">
											...
											@else
											<span class="symbol-label bg-light-danger text-danger fw-bold">{{substr($item->referee->name, 0,1)}}</span>
											@endif
										</div>
										<!--end::Avatar-->
									</div>
									...
									<div class="d-flex flex-column justify-content-center">
										<a href="" class="mb-1 text-gray-800 text-hover-primary">{{$item->referee->name}}</a>
										<div class="fw-bold fs-6 text-gray-400">{{$item->referee->email}}</div>
									</div>
									<!--end::Info-->
								</div>
								<!--end::User-->
							...
							<td>
								@if (is_null($item->status_id)) @php $i++; @endphp<span class="badge badge-light-danger fw-bolder px-4 py-3">In progress</span>
								@else <span class="badge badge-light-success fw-bolder px-4 py-3">Completed</span> @endif
								@if (is_null($item->status_id))
								<a href="{{route('email.referees', $item->id)}}" title="Resend Email to Referee" onclick="return confirm('Resend email. Are you sure?')" class="btn btn-sm btn-icon btn-light-info">
							...
							<td>
								@if (!getRefLog($item->referee_id, $item->talent_id)) @php $i++; @endphp<span class="badge badge-light-danger fw-bolder px-4 py-3">In progress</span>
								@else <span class="badge badge-light-success fw-bolder px-4 py-3">Completed</span> @endif
								@if (!is_null($item->status_id) && !getRefLog($item->referee_id, $item->talent_id))
								<a href="{{route('email.refereeassessment', $item->id)}}" title="Send Email to Referee" onclick="return confirm('Resend email. Are you sure?')" class="btn btn-sm btn-icon btn-light-info">
								<i class="bi bi-envelope-check-fill fs-5"></i>
								</a>
								@endif
							</td>
							<td class="text-end">
								<a href="{{route('remove.referees', $item->id)}}" title="Delete Referee" onclick="return confirm('Delete referee. Are you sure?')" class="btn btn-sm btn-icon @if($i<2) disabled btn-bg-light @else btn-light-danger @endif">
	...
	<div class="modal-dialog modal-dialog-centered mw-650px">
		...
		<div class="modal-content rounded">
			...
			<div class="modal-body scroll-y px-10 px-lg-15 pt-0 pb-15">
				<!--begin:Form-->
				<form id="kt_modal_new_target_form" class="form" action="{{route('create.referees')}}" method="POST">
					@csrf
					<!--begin::Heading-->
