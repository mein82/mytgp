...
<div class="card-header border-0 pt-6">
	...
	<div class="card-toolbar">
		...
		<div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
			...
			<a href="{{url('new/lock')}}" class="btn btn-primary">
...
<table id="kt_table_users" class="table table-rounded table-striped border gy-5 gs-5 table-row-bordered table-row-gray-200 fs-6">
    ...
    <tbody class="text-gray-800">
        ...
        @php $count++; @endphp
            <tr>
                <td>{{$count}}.</td>
                <td><span class="badge badge-secondary text-hover-gray-800">{{$item->type->name??''}}</span><br/>{{$item->name}}</td>
                <td>{{$item->start_date}}</td>
                <td>{{$item->end_date}}</td>
                <td>{{$item->users->count()}}</td>
                <td class="d-none">{{$item->users->pluck('user.name')}}</td>
                <td class="text-end">
                    ...
                    <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-125px py-4" data-kt-menu="true">
                        ...
                        <div class="menu-item px-3">
                            <a href="{{url('/lock/users/'.$item->id)}}" class="menu-link px-3">
                                Users
                            </a>
                        </div>
                        <!--end::Menu item-->
                        <!--begin::Menu item-->
                        <div class="menu-item px-3">
                            <a href="{{url('/edit/'.$item->id.'/lock')}}" class="menu-link px-3">
                                Edit
                            </a>
                        </div>
                        <!--end::Menu item-->
