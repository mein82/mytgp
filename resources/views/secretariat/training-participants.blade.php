				...
				<div id="kt_modal_users_search_handler" data-kt-search-keypress="true" data-kt-search-min-length="2" data-kt-search-enter="enter" data-kt-search-layout="inline">
					...
					<div class="py-5">
						<div data-kt-search-element="results" class="d-none">
							<form action="{{route('add.participants')}}" method="POST">
							@csrf<!--begin::Users-->
					...
					<tbody class="text-gray-700">
						...
						<tr>
							...
							<td class="text-gray-800">
								...
								{{$p->talent->user->name}}<br/>
                    			<span class="text-gray-700 fs-7">{{$p->talent->user->designation}}</span><br/>
                    			<span class="text-gray-500 fs-8">{{$p->talent->user->organization}}</span>
                    		</td>
                    		<td class="d-none">{{$p->talent->user->name}}</td>
							<!--end::Label-->
							<td class="text-center">
								<span class="badge badge-light-success mb-1">{{$p->talent->status->cohort}}</span>
								
							</td>
							<!--begin::Options-->
							<td class="kt-check">
								<input type="hidden" name="id[{{$p->talent->id}}]" value="{{$p->id}}">
								<!--begin::Wrapper-->
								<div class="d-flex">
									@foreach($status as $key=>$value)
									<!--begin::Checkbox-->
									<label class="form-check form-check-sm form-check-custom form-check-solid me-2">
										<input class="form-check-input" type="radio" value="{{$key}}" name="status_id[{{$p->id}}]" @if($p->status_id==$key) checked="" @endif/>
								...
								<div class="fv-row reason d-flex flex-wrap align-items-center">
									<div class="form-floating m-2">
										<!--begin::Input-->
										<input type="text" class="form-control" name="reason" value="{{$p->reason}}" id="floatingPassword">
										<!--end::Input-->
							...
							<td class="kt-check">
								...
								<div class="d-flex">
									...
									<!--begin::Checkbox-->
									<label class="form-check form-check-sm form-check-custom form-check-solid me-2">
										<input class="form-check-input" type="radio" value="{{$key}}" name="attendance_id[{{$p->id}}]" @if($p->attendance_id==$key) checked="" @endif/>
										<span class="form-check-label">{{$value}}</span>
										<span class="indicator-progress">
										<span class="spinner-border text-danger spinner-border-sm align-middle ms-2"></span>
    								</span>
								...
								<div class="fv-row remark d-flex flex-wrap align-items-center">
									<div class="form-floating m-2">
										<!--begin::Input-->
										<input type="text" class="form-control" name="remark" value="{{$p->remark}}" id="floatingPassword">
										<!--end::Input-->
							...
							<td class="d-none">{{$p->status->name??''}} </td>
							<td class="d-none">{{$p->reason}}</td>
							<td class="d-none">{{$p->attendance->name??''}} </td>
							<td class="d-none">{{$p->remark}}</td>
							<td class="d-none">{{($p->food==1)?'Vegetarian':''}}</td>
							<td class="d-none">{{($p->accomodation==1)?'Yes':''}}</td>
							<!--end::Options-->
						</tr>
						<!--end::Table row-->
						@endforeach
			...
			<div class="card-body pt-0 fs-5">
				...
				<div class="mb-7">
					...
					<div class="d-flex align-items-center mb-1">
						<!--begin::Name-->
						<span class="fw-bolder me-2 text-gray-800">{{$course->title}}</span>
						<!--end::Name-->
					</div>
					<!--end::Details-->
					<!--begin::Email-->
					<span class="badge badge-light">{{$course->domain->name??''}}</span>
					<!--end::Email-->
				</div>
				<!--end::Section-->
				<!--begin::Seperator-->
				...
				<div class="mb-7">
					...
					<div class="mb-0">
						<!--begin::Price-->
						<span class="fw-bold text-gray-800">{{ date("d M Y",strtotime($course->start_date))}}</span> - <span class="fw-bold text-gray-800">{{ date("d M Y",strtotime($course->end_date))}}</span>
						<span class="badge badge-light-success">{{$course->platform??''}}</span>
						<!--end::Price-->
					</div>
					<!--end::Details-->
				</div>
				...
				<div class="mb-10">
					...
					<div class="mb-0">
						<!--begin::Card info-->
						<div class="fw-bold d-flex align-items-center">{!!nl2br($course->remark)!!}</div>
						<!--end::Card info-->
					</div>
					<!--end::Details-->
				</div>
            ...
            document.querySelector('#kt_email_invite').addEventListener("click", (function() {
            	($('[type="checkbox"]:not(#all):checked').length==0)?Swal.fire({
                    ...
                    customClass: {
						...
						method: "POST",
						url: "{{route('emailinvite.talentcourse')}}",
						data: { id : yourArray, course: '{{$course->id}}'}
					}).done(function(){
						$('.process').removeAttr('data-kt-indicator'), Swal.fire({
