            ...
            <tr>
                ...
                <td>
                    ...
                    <div class="d-flex flex-column">
                        <span class="text-hover-primary mb-1">{{$item->super->name}}</span>
                        <span class="text-gray-600">{{$item->super->email}}</span>
                    </div>
                    <!--begin::User details-->
                </td>
                <!--end::user=-->
                <td>
                    <div class="d-flex flex-column">
                    <span class="mb-1">{{$item->super->designation}}</span>
                    <span class="text-gray-600">{{$item->super->organization}}</span>
                </div>
                </td>
                <!--begin::Action=-->
                <td>
                    ...
                    <div class="input-group">
                        <input type="text" name="remark" id="{{$item->id}}" value="{{$item->remark}}" class="form-control form-control-sm remark" placeholder="" aria-describedby="basic-addon2"/>
                ...
                <td class="text-center">
                    <input class="form-check-input active_super" type="radio" name="active_super" @if($item->status_id==1) checked @endif value="{{$item->super_id}}" id="flexRadioSm{{$item->super_id}}"  />
                </td>
                <td class="text-end">


                <button type="button" class="btn btn-light-danger btn-icon remove_super" id="kt_button_1" data-id="{{$item->id}}">
                    <span class="indicator-label">
        ...
        init: function() {
            ...
            (c = document.querySelectorAll(".remove_super"), c.forEach((e => {
                e.addEventListener("click", (function() {
                    ...
                    $.get( "{{url('talent/remove/supervisor')}}/"+id).done(function( data ) {
            ...
            (z = document.querySelectorAll(".active_super"), z.forEach((e => {
                e.addEventListener("click", (function() {
                    ...
                    $.get( "{{url('talent/activate')}}/{{$id}}/supervisor/"+super_id)
                }))
            }))),
            (d = document.querySelectorAll(".save"), d.forEach((e => {
                e.addEventListener("click", (function() {
                    ...
                    var val = parent.children[0].value;
                     $.ajax({
                      method: "POST",
                      url: "{{url('talent/remark/supervisor')}}",
                      data: { id: id, val: val }
                    }).done(function(){
...
KTUtil.onDOMContentLoaded((function() {
    ...
    $.post( "{{route('assign.supervisor')}}", $("#kt_form_super").serialize()).done(function( data ) {
        $.get( "{{url('talent/supervisor')}}/{{$id}}/modal", function( data ){
            $("#kt_modal_supervisor").find('.modal-load').html(data);
        });
    });
