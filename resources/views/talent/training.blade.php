...
<!--begin::Overlay-->
<a class="d-block overlay" data-fslightbox="lightbox-basic" href="{{url('')}}/img/training-2.png">
    <!--begin::Image-->
    <div class="overlay-wrapper bgi-no-repeat bgi-position-center bgi-size-cover card-rounded min-h-225px"
        style="background-size:50% ;background-image:url('{{url('')}}/img/training-2.png')">
    </div>
    <!--end::Image-->
...
<!--end::Overlay-->
<!--begin::Overlay-->
<a class="d-block overlay" data-fslightbox="lightbox-basic" href="{{url('')}}/img/training.png">
    <!--begin::Image-->
    <div class="overlay-wrapper bgi-no-repeat bgi-position-center bgi-size-cover card-rounded min-h-225px"
        style="background-image:url('{{url('')}}/img/training.png')">
    </div>
    <!--end::Image-->
    ...
    <table id="kt_table_users" class="table table-rounded table-striped border gy-5 gs-5 table-row-bordered table-row-gray-200 fs-6">
        ...
        <tbody class="text-gray-800">
            ...
            @php $count++; @endphp
                <tr>
                    <td>{{$count}}.</td>
                    <td><span class="badge badge-light-success me-2">{{$item->course->type->name??''}}</span><br/>{{$item->course->title}} <br/>@if(!is_null($item->course->platform_id))({{$item->course->platform??''}})@endif</td>
                    <td>{{ date("d/m/Y",strtotime($item->course->start_date))}} - {{ date("d/m/Y",strtotime($item->course->end_date))}}</td>
                    <td>{{$item->course->domain->name??''}}</td>
                    <td><div class="d-flex"> 
                        @if($item->food==1)
                        <!--begin::Symbol-->
                        ...
                        <span class="badge {{($item->status_id==1)?'badge-light-success':'badge-light-danger'}} me-2">{{$item->status->name??''}}</span>
                        
                        <div class="fs-7 ms-5 mt-2">{{$item->reason}}</div>
                    </td>
                    <td><span class="badge {{($item->attendance_id==1)?'badge-light-success':'badge-light-danger'}} me-2">{{$item->attendance->name??''}}</span>
                        <div class="fs-7 ms-5 mt-2">{{$item->remark}}</div>
                    </td>
                    <td class="text-end">
                        <a href="#" class="btn btn-primary @if($item->status_id>1 || $item->course->start_date<date('Y-m-d')) disabled @endif btn-active-light-primary btn-sm" data-bs-toggle="modal" data-bs-target="#kt_modal_response" data-id="{{$item->id}}">
                            Response
...
KTUtil.onDOMContentLoaded((function() {
    ...
    $("#kt_modal_response").on('show.bs.modal', function (e) {
        ...
        var modal = $(this);
        $.get( "{{url('training/response')}}/"+id+'/modal', function( data ){
