<table class="table table-rounded table-striped border table-row-bordered table-row-gray-200 gy-3 gx-5">
             		<tbody>
                @php $i = 0; @endphp        
             	@foreach($project->progress as $subitem)
                @php $i++; @endphp        
             	<tr><td colspan="2" class="fw-bold fs-6 gy-1">
             		<div class="d-flex flex-stack flex-wrap"> 
             		{{$i}}) {{ date("d-m-Y",strtotime($subitem->created_at))}} 
                    @if(Auth::user()->group_id==7)
             		<div class="d-flex justify-content-end">
                        <button type="button" class="btn btn-sm btn-light-danger btn-icon remove_super" id="kt_button_1" data-id="{{$subitem->id}}">
