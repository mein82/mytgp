	...
	<div class="card-body p-6 fs-6 table-responsive">
		...
		<div class="d-flex flex-wrap mb-10">
			<div class="form-check form-check-custom form-check-solid me-5 mb-5">
			    <input class="form-check-input" type="radio" name="user_type" id="flexRadioDefault1" value="1" <?php if($notify->public_id==1): ?> checked="" <?php endif; ?> />
			    <label class="form-check-label" for="flexRadioDefault1">
			        All
			    </label>
			</div>
			<div class="form-check form-check-custom form-check-solid me-5 mb-5">
			    <input class="form-check-input" type="radio" name="user_type" id="flexRadioDefault2" value="2" <?php if($notify->public_id==2): ?> checked="" <?php endif; ?> />
			    <label class="form-check-label" for="flexRadioDefault2">
			        Applicant
			    </label>
			</div>
			<div class="form-check form-check-custom form-check-solid me-5 mb-5">
			    <input class="form-check-input" type="radio" name="user_type" id="flexRadioDefault3" value="3" <?php if($notify->public_id==3): ?> checked="" <?php endif; ?> />
			    <label class="form-check-label" for="flexRadioDefault3">
			        Referee
			    </label>
			</div>
			<div class="form-check form-check-custom form-check-solid me-5 mb-5">
			    <input class="form-check-input" type="radio" name="user_type" id="flexRadioDefault4" value="4" <?php if($notify->public_id==4): ?> checked="" <?php endif; ?> />
			    <label class="form-check-label" for="flexRadioDefault4">
			        Talent
			    </label>
			</div>
			<div class="form-check form-check-custom form-check-solid me-5 mb-5">
			    <input class="form-check-input" type="radio" name="user_type" id="flexRadioDefault5" value="5" <?php if($notify->public_id==5): ?> checked="" <?php endif; ?> />
...
<?php echo Form::model($notify, array('route' => ['update.notification', $notify->id], 'method'=>'PATCH', 'id' => 'kt_add_course_form')); ?>
	...
	<tbody>
		<?php $__currentLoopData = $notify->notify; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $not): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
		<!--begin::User-->
<tr>
    <!--begin::ID-->
    <!--begin::User=-->
    <td>
		...
		<input type="hidden" name="user_id[]" value="<?php echo e($not->user->id); ?>">
	<!--end::Checkbox-->
	<!--begin::Avatar-->
	<div class="symbol symbol-35px symbol-circle">
		<?php if($not->user->avatar !='default.png'): ?>
		<img alt="Pic" src="<?php echo e(url('')); ?>/photo/<?php echo e($not->user->avatar); ?>" />
		<?php else: ?>
		<span class="symbol-label bg-light-danger text-danger fw-bold"><?php echo e(substr($not->user->name, 0,1)); ?></span>
		<?php endif; ?>
	</div>
	<!--end::Avatar-->
	<!--begin::Details-->
	<div class="ms-5">
		<a href="#" class="fs-5 fw-bolder text-gray-900 text-hover-primary mb-2"><?php echo e($not->user->name); ?></a>
		<div class="fw-bold text-muted"><?php echo e($not->user->email); ?></div>
	</div>
	<!--end::Details-->
</div>
</td>
...
<!--end::Details-->
<!--begin::Access menu-->
	<div class="fw-bold"><?php echo e($not->user->organization); ?></div>
	<div class="fw-bold text-muted"><?php echo e($not->user->contactno); ?></div>

</td>
<!--end::Access menu-->
<td class="text-end"><button type="button" class="btn btn-light-danger btn-icon remove_super" id="kt_button_1" data-id="<?php echo e($not->id); ?>">
            ...
            (e = document.querySelector("#kt_modal_users_search_handler")) && (e.querySelector('[data-kt-search-element="wrapper"]'), n = e.querySelector('[data-kt-search-element="results"]'), s = e.querySelector('[data-kt-search-element="empty"]'), (a = new KTSearch(e)).on("kt.search.process", r), a.on("kt.search.clear", o), document.querySelector('#kt_modal_users_search_reset').addEventListener("click", o), 
                ...
                (c = document.querySelectorAll(".remove_super"), c.forEach((e => {
                	...
                	var id = e.getAttribute('data-id');
	       				$.ajax({
		                  type: "DELETE",
		                  url: "<?php echo e(url('')); ?>/destroy/"+id+'/notify'
		                });
	                    var i = e.parentNode.parentNode.rowIndex;
	                    // console.log(i)
  						(i>-1)?document.getElementById("kt_roles_view_table").deleteRow(i):false;
...
<?php endif; ?><?php /**PATH /opt/homebrew/var/www/mytgp/resources/views/secretariat/notification-edit.blade.php ENDPATH**/ ?>